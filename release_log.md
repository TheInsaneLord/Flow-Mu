...
## Version 3.6 (Discord, Twitch, ai core, waffle_tts):
- Changes made to ai-core, twitch, Discord and waffle_tts bots.
  - made a list of settings to disable when changing channels
  - Made waffle save TTS for Flow-Mu lines to a separate location
  - Add STT for local use only
  - adjusted response to look 3 times for a reply message
  - Changed response check delay to 5 seconds
  - add a welcome message for new users
  - add a welcome back message for returning users

- bug fix:
  - Changed API key
  - Fixed Discord message tracking to get the correct response from AI
  - Fixed Twitch message tracking to get the correct response from AI

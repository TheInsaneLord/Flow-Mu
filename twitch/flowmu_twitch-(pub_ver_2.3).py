...
no_tts_channels = ["sniperheartwolf", "whaitjeezus"]
...
class Bot(commands.Bot):
    ...
    async def chat_log(self, message):
        if chat_history and ai_on and Botname.lower() in message.content.lower():
            ...
            ai_core.log_chat_message(conn, c, userid, username, content, platform='twitch')
        ...
        if ai_on:
            try:
                ...
                if not ai_core.has_responded_to_message(c, message.author.name):
                    ...
                    if chat_history:
                        ai_core.log_chat_message(conn, c, self.nick, Botname, response, is_response=True, platform='twitch')
...
#   info command
    ...
    async def info(self, ctx: commands.Context):
        await ctx.send(f'My name is Flow-Mu I am a AI bot that is a friend for the chat if you want to see how I work then use ?code I am currently in Version 2.3')
